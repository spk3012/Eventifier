...
class TabUpcomingEvents:Fragment(){

    private var adapterUpcomingEvent:AdapterUpcomingEvent?=null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        LoadEvent()
        var view = inflater.inflate(R.layout.tab_upcoming_events, container, false)
        var rv = view.findViewById(R.id.rvUpcoming) as RecyclerView

        adapterUpcomingEvent= AdapterUpcomingEvent(eventList, context!!)
        layoutManager= LinearLayoutManager(context)

        rv.layoutManager = layoutManager
        rv.adapter = adapterUpcomingEvent
        return view
    }

    fun LoadEvent(){

        myRef.child("Events").child("Verified Events")
